...
Type "msobol" in the "Set Permissions" search field. This will add "msobol" to the permissions list as a *Reader*. Next, select the drop-down menu where *Reader* is displayed and select *Editor*. This will update "msobol" to an *Editor*.

Notes:

- The "Set Permissions" search field allows you to search for a user or group by name, email or EDI identifier.
- The profiles shown in the "Select Resources" panel on the left are those specifically set for a given resource, while the list shown in the "Set Permissions" pane on the right is a combined list for all the selected resources.

### 5. Remove User from Data Package Permissions

To remove "msobol" (or any other user or group), select the resources for which the user or group should be removed, then select the permissions drop-down menu next to their name and select *None*.
