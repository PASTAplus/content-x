...
### Third-party identity provider user information

The Environmental Data Initiative utilizes third-party identity providers (Google, GitHub, and Orcid) to authenticate and uniquely identify customers who: (1) require EDI data repository privileges more significant than those provided to an anonymous (i.e., “public”) user but at a level less than that of an EDI LDAP account user for accessing restricted metadata and data; (2) use EDI’s “ezEML” metadata web application, which requires a unique user identifier for retaining application history; or (3) wish to create a unique customer profile within the suite of EDI applications. This form of identity authentication relies on OAuth2.0/OpenIdConnect protocols between EDI and the identity provider and does not store customer authentication (“sign-on”) credentials on any EDI host server. However, in addition to securely verifying your identity, it allows EDI access to minimal information the identity provider maintains about you: a string value that uniquely identifies you within their system (e.g., an email address) and your common name (if available). EDI may store this information in a web-based session cookie or database applications maintained by EDI to (1) provide a more consistent customer experience while visiting an EDI web application or (2) match your interaction with an EDI data product. By selecting authentication through a third-party identity provider, you consent to the release of the following information:

#### Google

The information requested about you from Google includes:
...
The information requested about you from GitHub includes:
...
The information requested about you from Orcid includes:
